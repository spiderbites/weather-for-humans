class Wearable < ActiveRecord::Base
  ...
  def self.need_extras(body_part, weather)
    result = Wearable.where("body_part ='#{body_part}' and conditions = #{weather.condition} and (min_temp <= #{weather.temperature} or min_temp = 'x') and (max_temp >= #{weather.temperature} or max_temp = 'x')")
    result.map { |wearable| wearable.clothing }
  end
